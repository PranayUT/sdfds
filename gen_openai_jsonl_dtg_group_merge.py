...
def process_episode(episode_file: str, base_dir: str) -> List[Dict[str, Any]]:
    """Process a single episode JSON file and return OpenAI format entries in 5-step chunks"""
    training_examples = []
    ...
    try:
        ...
        # Group steps into chunks of 5
        for i in range(0, len(steps), 5):
            chunk = steps[i:i+5]
            if len(chunk) < 5:  # skip incomplete chunks
                continue
            ...
            for step_data in chunk:
                ...
                # Validate paths
                if not os.path.exists(rgb_path) or not os.path.exists(map_path):
                    print(f"Warning: Missing images for step {step_data['step']} in episode {episode_id}")
                    continue
            ...
            # Only add if we got exactly 11 messages (1 system + 5 user + 5 assistant)
            if len(messages) == 11:
...
def main():
    ...
    output_file = "/store/rsiva/code/navid_ws/NaVid-VLN-CE/tmp/gpt-dataset/training_data_merged.jsonl"
